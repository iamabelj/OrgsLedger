...
if (!_isDev) {
    ...
    if (!exports.config.deepgram?.apiKey) {
        console.error('[CONFIG] FATAL: DEEPGRAM_API_KEY must be set in production for real-time transcription');
        process.exit(1);
    }
    if (!exports.config.livekit?.url || !exports.config.livekit?.apiKey || !exports.config.livekit?.apiSecret) {
