...
const mockDb = jest.fn((_table) => mockDbChain);
mockDb.fn = { now: jest.fn().mockReturnValue('NOW()') };
mockDb.raw = jest.fn((...args) => args);
...
describe('Tenant Isolation Middleware', () => {
    ...
    describe('loadMembership', () => {
        it('should reject when no orgId param', async () => {
            const req = createMockReq({ params: {}, user: { userId: 'u1', email: 'a@b.c', globalRole: 'member' } });
            const res = createMockRes();
            const next = jest.fn();
    ...
    describe('requireRole', () => {
        ...
        it('should allow exact role match', () => {
            const middleware = (0, rbac_1.requireRole)('executive');
            const req = createMockReq({
                user: { userId: 'u1', email: 'a@b.c', globalRole: 'member' },
        ...
        it('should allow higher role via hierarchy', () => {
            const middleware = (0, rbac_1.requireRole)('member');
            const req = createMockReq({
                user: { userId: 'u1', email: 'a@b.c', globalRole: 'member' },
        ...
        it('should accept any of multiple allowed roles', () => {
            const middleware = (0, rbac_1.requireRole)('org_admin', 'executive');
            const req = createMockReq({
                user: { userId: 'u1', email: 'a@b.c', globalRole: 'member' },
                membership: { id: 'm1', role: 'executive', organizationId: 'org-1', isActive: true },
            });
    ...
    describe('requireActiveSubscription', () => {
        it('should bypass for super_admin', async () => {
            ...
            const res = createMockRes();
            const next = jest.fn();
            await (0, subscription_1.requireActiveSubscription)(req, res, next);
            expect(next).toHaveBeenCalled();
        ...
        it('should allow active subscription through', async () => {
            ...
            await (0, subscription_1.requireActiveSubscription)(req, res, next);
            expect(next).toHaveBeenCalled();
            expect(req.subscription).toEqual(sub);
        });
        ...
        it('should use organizationId from request if orgId param missing', async () => {
            ...
            req.organizationId = 'org-from-body';
            const res = createMockRes();
            const next = jest.fn();
            await (0, subscription_1.requireActiveSubscription)(req, res, next);
    ...
    describe('checkAiWallet', () => {
        it('should set wallet balance on request', async () => {
            ...
            const res = createMockRes();
            const next = jest.fn();
            await (0, subscription_1.checkAiWallet)(req, res, next);
            expect(req.aiWalletBalance).toBe(120.5);
            expect(req.aiWalletEmpty).toBe(false);
            expect(next).toHaveBeenCalled();
        });
        it('should mark wallet as empty when balance is 0', async () => {
            ...
            const next = jest.fn();
            await (0, subscription_1.checkAiWallet)(req, res, next);
            expect(req.aiWalletEmpty).toBe(true);
            expect(next).toHaveBeenCalled();
        });
    ...
    describe('checkTranslationWallet', () => {
        it('should set wallet balance on request', async () => {
            ...
            const res = createMockRes();
            const next = jest.fn();
            await (0, subscription_1.checkTranslationWallet)(req, res, next);
            expect(req.translationWalletBalance).toBe(60);
            expect(req.translationWalletEmpty).toBe(false);
            expect(next).toHaveBeenCalled();
        });
        it('should mark wallet as empty when balance is negative', async () => {
            ...
            const next = jest.fn();
            await (0, subscription_1.checkTranslationWallet)(req, res, next);
            expect(req.translationWalletEmpty).toBe(true);
            expect(next).toHaveBeenCalled();
        });
    });
    // ── loadMembershipAndSub (combined) ─────────────────────
