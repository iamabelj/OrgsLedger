...
describe('Stress: Wallet Deductions Under Concurrency', () => {
    ...
    async function simulateDeduction(orgId, minutes, desc) {
    ...
    it('should handle 50 concurrent 10-minute deductions from 1000-min wallet', async () => {
        ...
        const promises = Array.from({ length: DEDUCTION_COUNT }, (_, i) => simulateDeduction('org-1', MINUTES_PER, `Deduction ${i}`));
    ...
    it('should maintain exact balance after 100 mixed-size deductions', async () => {
        ...
        const promises = deductions.map((minutes, i) => simulateDeduction('org-1', minutes, `Mixed deduction ${i}`));
        const results = await Promise.all(promises);
        const successes = results.filter((r) => r.success).length;
    ...
    it('should prevent double-spending with lock contention', async () => {
        ...
        const promises = Array.from({ length: 10 }, (_, i) => simulateDeduction('org-1', 10, `Double-spend test ${i}`));
        const results = await Promise.all(promises);
        const successes = results.filter((r) => r.success).length;
        const failures = results.filter((r) => !r.success).length;
    ...
    it('should calculate AI meeting costs correctly under load', async () => {
        ...
        const meetingDurations = Array.from({ length: MEETING_COUNT }, (_, i) => 15 + (i * 5) + (i % 3) * 10);
        const promises = meetingDurations.map((duration, i) => simulateDeduction('org-1', duration * AI_RATE_PER_MINUTE, `Meeting ${i}: ${duration} min`));
        const results = await Promise.all(promises);
    ...
    it('should handle fractional minute deductions without rounding errors', async () => {
        ...
        const promises = Array.from({ length: 10 }, (_, i) => simulateDeduction('org-1', 10.05, `Fractional ${i}`));
        const results = await Promise.all(promises);
        const successes = results.filter((r) => r.success).length;
    ...
    it('should use db.raw for atomic balance updates (not read-modify-write)', () => {
        ...
        const mockRaw = jest.fn((sql, bindings) => ({ sql, bindings }));
    ...
    it('should handle 20 concurrent top-ups correctly', async () => {
        ...
        async function simulateTopUp(orgId, minutes) {
            await acquireLock();
        ...
        const promises = Array.from({ length: 20 }, (_, i) => simulateTopUp('org-1', 60));
        const results = await Promise.all(promises);
    ...
    it('should isolate wallet operations across 10 different organizations', async () => {
        ...
        async function orgDeduct(orgId, minutes) {
            const lock = orgLocks[orgId];
            // Acquire per-org lock
            if (lock.locked) {
                await new Promise((resolve) => lock.queue.push(() => { lock.locked = true; resolve(); }));
            }
            else {
                lock.locked = true;
            }
    ...
    it('should handle AI and translation wallets independently under load', async () => {
        ...
        const aiQueue = [];
        const trQueue = [];
        async function deductAI(minutes) {
            if (aiLocked)
                await new Promise(r => aiQueue.push(() => { aiLocked = true; r(); }));
            else
                aiLocked = true;
            if (aiBalance < minutes) {
                aiLocked = false;
                if (aiQueue.length)
                    aiQueue.shift()();
                return { success: false, type: 'ai' };
            }
            aiBalance -= minutes;
            aiLocked = false;
            if (aiQueue.length)
        ...
        async function deductTranslation(minutes) {
            if (trLocked)
                await new Promise(r => trQueue.push(() => { trLocked = true; r(); }));
            else
                trLocked = true;
            if (translationBalance < minutes) {
                trLocked = false;
                if (trQueue.length)
                    trQueue.shift()();
                return { success: false, type: 'translation' };
            }
...
//# sourceMappingURL=stress-wallet-concurrency.test.js.map