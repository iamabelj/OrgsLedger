...
router.get('/admin/revenue', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (_req, res) => {
    try {
        ...
        const byPlan = await (0, db_1.default)('subscriptions')
            .join('subscription_plans', 'subscriptions.plan_id', 'subscription_plans.id')
            .where('subscriptions.amount_paid', '>', 0)
            .groupBy('subscription_plans.slug', 'subscriptions.currency')
            .select('subscription_plans.slug as plan', 'subscriptions.currency', db_1.default.raw('COUNT(*) as count'), db_1.default.raw('COALESCE(SUM(subscriptions.amount_paid), 0) as revenue'));
...
router.get('/admin/subscriptions', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        ...
        const subs = await (0, db_1.default)('subscriptions')
            ...
            .leftJoin((0, db_1.default)('memberships').groupBy('organization_id').select('organization_id').count('* as member_count').as('mc'), 'organizations.id', 'mc.organization_id')
            .select('subscriptions.id', 'subscriptions.organization_id', 'subscriptions.status', 'subscriptions.billing_cycle', 'subscriptions.currency', 'subscriptions.amount_paid', 'subscriptions.current_period_end', 'subscriptions.current_period_start', 'subscriptions.grace_period_end', 'subscriptions.created_at', 'subscription_plans.name as plan_name', 'subscription_plans.slug as plan', 'organizations.name as org_name', 'organizations.subscription_status', db_1.default.raw('COALESCE(mc.member_count, 0) as member_count'))
    ...
    try {
        ...
        const baseQuery = (0, db_1.default)('organizations')
          ...
          ORDER BY organization_id, created_at DESC
            ...
            .leftJoin(db_1.default.raw("wallet as ai_wallet ON organizations.id = ai_wallet.organization_id AND ai_wallet.service_type = 'ai'"))
            .leftJoin(db_1.default.raw("wallet as translation_wallet ON organizations.id = translation_wallet.organization_id AND translation_wallet.service_type = 'translation'"));
...
router.get('/admin/wallet-analytics', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (_req, res) => {
    try {
        ...
        const perOrg = await (0, db_1.default)('organizations')
            .leftJoin(db_1.default.raw("wallet as ai_wallet ON organizations.id = ai_wallet.organization_id AND ai_wallet.service_type = 'ai'"))
            .leftJoin(db_1.default.raw("wallet as translation_wallet ON organizations.id = translation_wallet.organization_id AND translation_wallet.service_type = 'translation'"))
            .select('organizations.id', 'organizations.name', 'organizations.billing_currency', db_1.default.raw('COALESCE(ai_wallet.balance_minutes, 0) as ai_balance_minutes'), db_1.default.raw('COALESCE(translation_wallet.balance_minutes, 0) as translation_balance_minutes'))
            .orderBy('organizations.name');
        // Add usage stats per org
        const orgIds = perOrg.map((o) => o.id);
        let aiUsageByOrg = {};
        let transUsageByOrg = {};
        if (orgIds.length > 0) {
            const aiUsageRows = await (0, db_1.default)('wallet_transactions')
                .whereIn('organization_id', orgIds)
                .where({ service_type: 'ai' })
                .where('amount_minutes', '<', 0)
                .groupBy('organization_id')
                .select('organization_id', db_1.default.raw('SUM(ABS(amount_minutes)) as used_minutes'));
            aiUsageRows.forEach((r) => { aiUsageByOrg[r.organization_id] = parseFloat(r.used_minutes || 0); });
            const transUsageRows = await (0, db_1.default)('wallet_transactions')
                .whereIn('organization_id', orgIds)
                .where({ service_type: 'translation' })
                .where('amount_minutes', '<', 0)
                .groupBy('organization_id')
                .select('organization_id', db_1.default.raw('SUM(ABS(amount_minutes)) as used_minutes'));
            transUsageRows.forEach((r) => { transUsageByOrg[r.organization_id] = parseFloat(r.used_minutes || 0); });
        }
        const organizations = perOrg.map((o) => ({
            id: o.id,
            org_id: o.id,
            name: o.name,
            org_name: o.name,
    ...
    try {
        ...
        const counts = planIds.length > 0
            ? await (0, db_1.default)('subscriptions')
                ...
                .groupBy('plan_id')
                .select('plan_id', db_1.default.raw('COUNT(*) as subscriber_count'))
            : [];
        const countMap = {};
        counts.forEach((c) => { countMap[c.plan_id] = parseInt(c.subscriber_count); });
...
const createPlanSchema = zod_1.z.object({
    ...
    maxMembers: zod_1.z.number().int().min(1).default(100),
    priceUsdAnnual: zod_1.z.number().min(0).default(0),
    priceUsdMonthly: zod_1.z.number().min(0).optional(),
    priceNgnAnnual: zod_1.z.number().min(0).default(0),
...
router.post('/admin/plans', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), (0, middleware_1.validate)(createPlanSchema), async (req, res) => {
    try {
        ...
        // Check slug uniqueness
        const existing = await (0, db_1.default)('subscription_plans').where({ slug }).first();
        if (existing) {
        ...
        logger_1.logger.info(`Created subscription plan: ${name} (${slug})`);
        res.status(201).json({ success: true, data: plan });
    }
    catch (err) {
        logger_1.logger.error('Create plan error', err);
...
router.delete('/admin/plans/:planId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        const plan = await (0, db_1.default)('subscription_plans').where({ id: req.params.planId }).first();
        if (!plan) {
            res.status(404).json({ success: false, error: 'Plan not found' });
        ...
        const activeCount = await (0, db_1.default)('subscriptions')
        ...
        if (parseInt(activeCount?.count) > 0) {
            ...
            await (0, db_1.default)('subscription_plans').where({ id: req.params.planId }).update({ is_active: false, updated_at: new Date() });
            await (0, audit_1.writeAuditLog)({
                userId: req.user.userId,
                action: 'archive',
                entityType: 'subscription_plan',
        ...
        await (0, db_1.default)('subscription_plans').where({ id: req.params.planId }).delete();
        await (0, audit_1.writeAuditLog)({
            userId: req.user.userId,
            action: 'delete',
            entityType: 'subscription_plan',
        ...
        logger_1.logger.info(`Deleted subscription plan: ${plan.name} (${plan.slug})`);
        res.json({ success: true, message: 'Plan deleted' });
    }
    catch (err) {
        logger_1.logger.error('Delete plan error', err);
...
router.get('/admin/organizations/:orgId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        ...
        const plan = subscription
            ? await (0, db_1.default)('subscription_plans').where({ id: subscription.plan_id }).first()
            : null;
        // Get wallets
        const aiWallet = await (0, db_1.default)('wallet').where({ organization_id: req.params.orgId, service_type: 'ai' }).first();
        const translationWallet = await (0, db_1.default)('wallet').where({ organization_id: req.params.orgId, service_type: 'translation' }).first();
        ...
        logger_1.logger.error('Get org detail error', err);
...
const updateOrgSchema = zod_1.z.object({
    ...
    subscriptionStatus: zod_1.z.enum(['active', 'grace_period', 'expired', 'cancelled', 'suspended']).optional(),
    billingCurrency: zod_1.z.enum(['USD', 'NGN']).optional(),
    billingCountry: zod_1.z.string().max(5).optional(),
    settings: zod_1.z.record(zod_1.z.any()).optional(),
});
router.put('/admin/organizations/:orgId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), (0, middleware_1.validate)(updateOrgSchema), async (req, res) => {
    try {
        ...
        if (slug && slug !== previous.slug) {
            const existing = await (0, db_1.default)('organizations').where({ slug }).whereNot({ id: req.params.orgId }).first();
            if (existing) {
        ...
        if (billingCurrency !== undefined)
            updates.billing_currency = billingCurrency;
        ...
        if (subscriptionStatus === 'active') {
            const existingSub = await (0, db_1.default)('subscriptions')
                .where({ organization_id: req.params.orgId })
                .orderBy('created_at', 'desc')
                .first();
            if (existingSub) {
                ...
                if (now > periodEnd) {
                    ...
                    await (0, db_1.default)('subscriptions').where({ id: existingSub.id }).update({
                        status: 'active',
                    ...
                    logger_1.logger.info(`Auto-renewed subscription for org ${req.params.orgId} (was expired)`);
                }
                else {
        ...
        const updated = await (0, db_1.default)('organizations').where({ id: req.params.orgId }).first();
        logger_1.logger.info(`Admin updated organization: ${updated.name} (${updated.slug})`);
        res.json({ success: true, data: updated });
    }
    catch (err) {
        logger_1.logger.error('Update org error', err);
...
const assignPlanSchema = zod_1.z.object({
    planSlug: zod_1.z.string().min(1).max(100),
    billingCycle: zod_1.z.enum(['annual', 'monthly']).default('annual'),
    currency: zod_1.z.enum(['USD', 'NGN']).default('USD'),
});
router.post('/admin/organizations/:orgId/assign-plan', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), (0, middleware_1.validate)(assignPlanSchema), async (req, res) => {
    try {
        const org = await (0, db_1.default)('organizations').where({ id: req.params.orgId }).first();
        if (!org) {
            res.status(404).json({ success: false, error: 'Organization not found' });
        ...
        logger_1.logger.info(`Admin assigned plan "${plan.name}" to org "${org.name}" (${org.slug})`);
...
router.delete('/admin/organizations/:orgId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        const org = await (0, db_1.default)('organizations').where({ id: req.params.orgId }).first();
        if (!org) {
            res.status(404).json({ success: false, error: 'Organization not found' });
        ...
        await (0, db_1.default)('organizations').where({ id: req.params.orgId }).delete();
        logger_1.logger.warn(`Admin DELETED organization: ${org.name} (${org.slug}) by user ${req.user.userId}`);
        res.json({ success: true, message: `Organization "${org.name}" deleted` });
    }
    catch (err) {
        logger_1.logger.error('Delete org error', err);
...
router.get('/admin/users', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = Math.min(parseInt(req.query.limit) || 50, 200);
        const search = req.query.search;
        const globalRole = req.query.globalRole;
        let query = (0, db_1.default)('users')
            .select('users.id', 'users.email', 'users.first_name', 'users.last_name', 'users.global_role', 'users.email_verified', 'users.created_at', 'users.last_login_at', db_1.default.raw('(SELECT COUNT(*) FROM memberships WHERE memberships.user_id = users.id AND memberships.is_active = true) as org_count'));
        if (search) {
...
router.get('/admin/users/:userId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        ...
        const memberships = await (0, db_1.default)('memberships')
            ...
            .select('memberships.id', 'memberships.role', 'memberships.is_active', 'memberships.joined_at', 'organizations.id as org_id', 'organizations.name as org_name', 'organizations.slug as org_slug');
        ...
        logger_1.logger.error('Get user detail error', err);
...
const updateUserSchema = zod_1.z.object({
    ...
    globalRole: zod_1.z.enum(['member', 'developer', 'super_admin']).optional(),
    isVerified: zod_1.z.boolean().optional(),
});
router.put('/admin/users/:userId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), (0, middleware_1.validate)(updateUserSchema), async (req, res) => {
    try {
        const previous = await (0, db_1.default)('users').where({ id: req.params.userId }).first();
        if (!previous) {
            res.status(404).json({ success: false, error: 'User not found' });
            return;
        }
        ...
        if (globalRole !== undefined)
            updates.global_role = globalRole;
        if (isVerified !== undefined)
            updates.email_verified = isVerified;
        await (0, db_1.default)('users').where({ id: req.params.userId }).update(updates);
        await (0, audit_1.writeAuditLog)({
            userId: req.user.userId,
        ...
        const updated = await (0, db_1.default)('users')
            .where({ id: req.params.userId })
            .select('id', 'email', 'first_name', 'last_name', 'global_role', 'email_verified')
            .first();
        logger_1.logger.info(`Admin updated user: ${updated.email} - role: ${updated.global_role}`);
        res.json({ success: true, data: updated });
    }
    catch (err) {
        logger_1.logger.error('Update user error', err);
...
router.get('/admin/audit-logs', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        ...
        let query = (0, db_1.default)('audit_logs')
            ...
            .select('audit_logs.id', 'audit_logs.action', 'audit_logs.entity_type', 'audit_logs.entity_id', 'audit_logs.ip_address', 'audit_logs.created_at', 'audit_logs.user_id', 'audit_logs.organization_id', db_1.default.raw("COALESCE(users.first_name || ' ' || users.last_name, users.email, 'System') as user_name"), db_1.default.raw("COALESCE(organizations.name, '') as org_name"));
        if (action)
        ...
        logger_1.logger.error('Admin audit logs error', err);
...
router.get('/admin/risk/low-balances', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        ...
        const lowAi = await (0, db_1.default)('wallet')
            ...
            .select('organizations.id as org_id', 'organizations.name as org_name', 'wallet.balance_minutes', db_1.default.raw("'ai' as wallet_type"))
            .orderBy('wallet.balance_minutes', 'asc');
        const lowTranslation = await (0, db_1.default)('wallet')
            .join('organizations', 'wallet.organization_id', 'organizations.id')
            .where({ 'wallet.service_type': 'translation' })
            .where('wallet.balance_minutes', '<', thresholdMinutes)
            .where('wallet.balance_minutes', '>', 0)
            .select('organizations.id as org_id', 'organizations.name as org_name', 'wallet.balance_minutes', db_1.default.raw("'translation' as wallet_type"))
            .orderBy('wallet.balance_minutes', 'asc');
        const emptyAi = await (0, db_1.default)('wallet')
            .join('organizations', 'wallet.organization_id', 'organizations.id')
            .where({ 'wallet.service_type': 'ai' })
            .where('wallet.balance_minutes', '<=', 0)
            .select('organizations.id as org_id', 'organizations.name as org_name', 'wallet.balance_minutes', db_1.default.raw("'ai' as wallet_type"));
        const emptyTranslation = await (0, db_1.default)('wallet')
            .join('organizations', 'wallet.organization_id', 'organizations.id')
            .where({ 'wallet.service_type': 'translation' })
            .where('wallet.balance_minutes', '<=', 0)
            .select('organizations.id as org_id', 'organizations.name as org_name', 'wallet.balance_minutes', db_1.default.raw("'translation' as wallet_type"));
        ...
        for (const w of [...lowAi, ...lowTranslation, ...emptyAi, ...emptyTranslation]) {
            if (!orgMap[w.org_id])
                orgMap[w.org_id] = { id: w.org_id, org_id: w.org_id, name: w.org_name, org_name: w.org_name, ai_balance: 0, translation_balance: 0, ai_balance_hours: 0, translation_balance_hours: 0, type: 'low' };
            const mins = parseFloat(w.balance_minutes);
            const hours = mins / 60;
            if (w.wallet_type === 'ai') {
                orgMap[w.org_id].ai_balance = mins;
                orgMap[w.org_id].ai_balance_hours = hours;
            }
            else {
                orgMap[w.org_id].translation_balance = mins;
                orgMap[w.org_id].translation_balance_hours = hours;
            }
            if (hours <= 0)
                orgMap[w.org_id].type = 'critical';
        }
...
router.get('/admin/risk/spikes', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        ...
        const aiDaily = await (0, db_1.default)('wallet_transactions')
            .where({ service_type: 'ai' })
            .where('amount_minutes', '<', 0)
        ...
        const transDaily = await (0, db_1.default)('wallet_transactions')
            .where({ service_type: 'translation' })
            .where('amount_minutes', '<', 0)
            .where('created_at', '>=', db_1.default.raw("NOW() - make_interval(days := ?)", [lookbackDays]))
            .select('organization_id', db_1.default.raw("DATE(created_at) as day"), db_1.default.raw('SUM(ABS(amount_minutes)) as daily_usage'))
...
// ════════════════════════════════════════════════════════════
const createSignupInviteSchema = zod_1.z.object({
    email: zod_1.z.string().email().optional().nullable(),
    role: zod_1.z.enum(['member', 'executive', 'org_admin']).default('member'),
    organizationId: zod_1.z.string().uuid().optional().nullable(),
    maxUses: zod_1.z.number().int().min(1).default(1),
    expiresInDays: zod_1.z.number().int().min(1).optional().nullable(),
...
router.post('/admin/signup-invites', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), (0, middleware_1.validate)(createSignupInviteSchema), async (req, res) => {
    try {
        ...
        const expiresAt = expiresInDays
            ...
            : null;
        const [invite] = await (0, db_1.default)('signup_invites').insert({
            code,
        ...
        if (email) {
          ...
          `,
                ...
                logger_1.logger.info(`Signup invite email sent to ${email} with code ${code}`);
            }
            catch (emailErr) {
                logger_1.logger.warn('Failed to send signup invite email:', emailErr);
            }
        }
        await (0, audit_1.writeAuditLog)({
            userId: req.user.userId,
        ...
        logger_1.logger.info(`Signup invite created: ${code} by ${req.user.userId}`);
        res.status(201).json({ success: true, data: { ...invite, inviteUrl } });
    }
    catch (err) {
        logger_1.logger.error('Create signup invite error', err);
        res.status(500).json({ success: false, error: 'Failed to create signup invite' });
    }
});
// GET /admin/signup-invites — List all signup invites
router.get('/admin/signup-invites', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 50;
        const offset = (page - 1) * limit;
        const status = req.query.status; // 'active', 'expired', 'used', 'all'
        let query = (0, db_1.default)('signup_invites')
            ...
            .select('signup_invites.*', 'organizations.name as organization_name', 'organizations.slug as organization_slug', 'creator.first_name as creator_first_name', 'creator.last_name as creator_last_name');
        if (status === 'active') {
        ...
        const invites = await query
            ...
            .limit(limit)
            .offset(offset);
        const [{ count }] = await (0, db_1.default)('signup_invites').count('* as count');
        res.json({ success: true, data: invites, total: parseInt(count), page, limit });
    }
    catch (err) {
        logger_1.logger.error('List signup invites error', err);
...
router.delete('/admin/signup-invites/:inviteId', middleware_1.authenticate, (0, middleware_1.requireDeveloper)(), async (req, res) => {
    try {
        await (0, db_1.default)('signup_invites').where({ id: req.params.inviteId }).update({ is_active: false });
        res.json({ success: true, message: 'Signup invite deactivated' });
    }
    catch (err) {
        logger_1.logger.error('Delete signup invite error', err);
...
router.get('/invite/validate/:code', async (req, res) => {
    try {
        ...
        const invite = await (0, db_1.default)('signup_invites').where({ code, is_active: true }).first();
        if (!invite) {
        ...
        if (invite.organization_id) {
            const org = await (0, db_1.default)('organizations').where({ id: invite.organization_id }).first();
