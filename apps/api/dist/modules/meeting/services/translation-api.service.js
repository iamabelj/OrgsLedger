...
exports.translationApiService = exports.SUPPORTED_LANGUAGES = void 0;
...
class TranslationApiService {
    ...
    async translate(text, targetLang, sourceLang = 'en') {
        ...
        if (cached) {
            logger_1.logger.debug('[TRANSLATION_API] Cache hit', { srcLang, tgtLang, textLength: text.length });
            return { translatedText: cached, sourceLang: srcLang, targetLang: tgtLang };
        }
        // Get provider from config
        const provider = config_1.config.translation?.provider || 'free';
        let translatedText;
        try {
            switch (provider) {
                case 'google':
            ...
            logger_1.logger.debug('[TRANSLATION_API] Translation completed', {
                provider,
                srcLang,
                tgtLang,
            ...
            logger_1.logger.error('[TRANSLATION_API] Translation failed', {
                provider,
                srcLang,
                tgtLang,
                error: err.message,
    ...
    async translateWithGoogle(text, sourceLang, targetLang) {
        try {
            ...
            logger_1.logger.error('[TRANSLATION_API] Google Translate failed', {
                error: err.message,
                sourceLang,
                targetLang,
            });
    ...
    async translateWithDeepL(text, sourceLang, targetLang) {
        ...
        if (!apiKey) {
            logger_1.logger.warn('[TRANSLATION_API] DEEPL_API_KEY not set, falling back to free API');
            return this.translateWithFreeApi(text, sourceLang, targetLang);
        }
        try {
            // Dynamic import
            ...
            const result = await translator.translateText(text, sourceLang.toLowerCase(), targetLang.toUpperCase());
            return result.text;
        }
        catch (err) {
            logger_1.logger.error('[TRANSLATION_API] DeepL failed', {
                error: err.message,
                sourceLang,
                targetLang,
            });
        ...
        try {
            ...
            if (data.responseStatus === 200 && data.responseData?.translatedText) {
                ...
                if (translated !== 'UNTRANSLATED' && !translated.includes('QUERY LENGTH LIMIT')) {
                    logger_1.logger.debug('[TRANSLATION_API] Free API translation success', { sourceLang, targetLang });
                    return translated;
                }
            }
            // If the response has matches, use the best one
            if (data.matches && data.matches.length > 0) {
                const bestMatch = data.matches.reduce((best, m) => m.quality > (best?.quality || 0) ? m : best, data.matches[0]);
            ...
            logger_1.logger.error('[TRANSLATION_API] Free API failed', {
                error: err.message,
                sourceLang,
                targetLang,
            });
...
// ── Singleton Export ────────────────────────────────────────
exports.translationApiService = new TranslationApiService();
