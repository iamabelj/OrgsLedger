...
exports.meetingAICostProtector = exports.aiMinutesDisabledGauge = exports.aiCostUsedGauge = exports.aiTokensUsedGauge = exports.aiCostLimitHitsCounter = void 0;
exports.initializeMeetingAICostProtector = initializeMeetingAICostProtector;
exports.checkAIUsage = checkAIUsage;
exports.recordAIUsage = recordAIUsage;
exports.getMeetingAIUsage = getMeetingAIUsage;
exports.isMinutesGenerationDisabled = isMinutesGenerationDisabled;
exports.resetMeetingAIUsage = resetMeetingAIUsage;
exports.getAICostConfig = getAICostConfig;
...
class MeetingAICostProtector {
    ...
    async initialize() {
        ...
        try {
            ...
            logger_1.logger.info('[AI_COST] Initialized', {
            ...
            logger_1.logger.error('[AI_COST] Failed to initialize', {
     ...
     */
    async checkUsage(meetingId, estimatedTokens) {
        ...
        if (estimatedTokens) {
            ...
            if (projectedTotal > AI_COST_CONFIG.maxTokensPerMeeting) {
                logger_1.logger.warn('[AI_COST] AI_COST_LIMIT_REACHED (projected)', {
                    meetingId,
        ...
        if (usage.percentUsed >= AI_COST_CONFIG.warningThresholdPercent) {
            logger_1.logger.warn('[AI_COST] Approaching token limit', {
                meetingId,
     ...
     */
    async recordUsage(meetingId, inputTokens, outputTokens, organizationId) {
        ...
        try {
            ...
            const newTotalTokens = results?.[0]?.[1] || totalTokens;
            // Calculate usage
            const usage = this.calculateUsage(meetingId, newTotalTokens, inputTokens, outputTokens);
            // Update metrics
            exports.aiTokensUsedGauge.set({ meeting_id: meetingId }, newTotalTokens);
            exports.aiCostUsedGauge.set({ meeting_id: meetingId }, usage.estimatedCostUsd);
            // Check if limit reached
            if (newTotalTokens >= AI_COST_CONFIG.maxTokensPerMeeting) {
                logger_1.logger.warn('[AI_COST] AI_COST_LIMIT_REACHED', {
                    meetingId,
                    totalTokens: newTotalTokens,
                    limit: AI_COST_CONFIG.maxTokensPerMeeting,
                });
                await this.disableMinutes(meetingId);
                exports.aiCostLimitHitsCounter.inc();
                exports.aiMinutesDisabledGauge.set({ meeting_id: meetingId }, 1);
            ...
            if (AI_COST_CONFIG.persistUsage) {
                ...
                if (this.usageBatch.length >= AI_COST_CONFIG.persistBatchSize) {
                    this.flushUsageBatch().catch((err) => {
                        logger_1.logger.error('[AI_COST] Failed to flush batch', { error: err });
                    });
                }
            }
            return usage;
            ...
            logger_1.logger.error('[AI_COST] Failed to record usage', {
                meetingId,
                inputTokens,
                outputTokens,
                error: err instanceof Error ? err.message : String(err),
    ...
    async getUsage(meetingId) {
        ...
        try {
            ...
            const results = await pipeline.exec();
            const totalTokens = parseInt(results?.[0]?.[1] || '0', 10);
            const inputTokens = parseInt(results?.[1]?.[1] || '0', 10);
            const outputTokens = parseInt(results?.[2]?.[1] || '0', 10);
            const isDisabled = results?.[3]?.[1] === 1;
            return this.calculateUsage(meetingId, totalTokens, inputTokens, outputTokens, isDisabled);
        }
        catch (err) {
            logger_1.logger.error('[AI_COST] Failed to get usage', {
                meetingId,
                error: err instanceof Error ? err.message : String(err),
            });
     ...
     */
    async disableMinutes(meetingId) {
        const redis = this.getRedis();
        await redis.setex(getDisabledKey(meetingId), 86400, '1');
        exports.aiMinutesDisabledGauge.set({ meeting_id: meetingId }, 1);
        logger_1.logger.warn('[AI_COST] Minutes generation disabled for meeting', {
            meetingId,
            reason: 'AI_COST_LIMIT_REACHED',
        });
    }
     ...
     */
    async enableMinutes(meetingId) {
        const redis = this.getRedis();
        await redis.del(getDisabledKey(meetingId));
        exports.aiMinutesDisabledGauge.set({ meeting_id: meetingId }, 0);
        logger_1.logger.info('[AI_COST] Minutes generation re-enabled', { meetingId });
    }
    /**
     * Check if minutes generation is disabled.
     */
    async isMinutesDisabled(meetingId) {
        const redis = this.getRedis();
    ...
    async reset(meetingId) {
        const redis = this.getRedis();
        try {
            await redis.del(getTokensKey(meetingId), getInputTokensKey(meetingId), getOutputTokensKey(meetingId), getDisabledKey(meetingId));
            // Clear metrics
            exports.aiTokensUsedGauge.remove({ meeting_id: meetingId });
            exports.aiCostUsedGauge.remove({ meeting_id: meetingId });
            exports.aiMinutesDisabledGauge.remove({ meeting_id: meetingId });
            logger_1.logger.debug('[AI_COST] Reset AI usage', { meetingId });
        }
        catch (err) {
            logger_1.logger.error('[AI_COST] Failed to reset', {
                meetingId,
                error: err instanceof Error ? err.message : String(err),
            });
        }
     ...
     */
    calculateUsage(meetingId, totalTokens, inputTokens, outputTokens, isDisabled) {
    ...
    async flushUsageBatch() {
        ...
        `, [usage.meetingId, usage.organizationId || null, usage.inputTokens, usage.outputTokens]);
            ...
            logger_1.logger.debug('[AI_COST] Flushed usage batch', {
            ...
            logger_1.logger.error('[AI_COST] Failed to persist usage', {
                error: err instanceof Error ? err.message : String(err),
    ...
    async shutdown() {
        ...
        logger_1.logger.info('[AI_COST] Shutdown complete');
    }
}
// ── Singleton ───────────────────────────────────────────────
exports.meetingAICostProtector = new MeetingAICostProtector();
// ── Exports ─────────────────────────────────────────────────
async function initializeMeetingAICostProtector() {
    await exports.meetingAICostProtector.initialize();
}
async function checkAIUsage(meetingId, estimatedTokens) {
    return exports.meetingAICostProtector.checkUsage(meetingId, estimatedTokens);
}
async function recordAIUsage(meetingId, inputTokens, outputTokens, organizationId) {
    return exports.meetingAICostProtector.recordUsage(meetingId, inputTokens, outputTokens, organizationId);
}
async function getMeetingAIUsage(meetingId) {
    return exports.meetingAICostProtector.getUsage(meetingId);
}
async function isMinutesGenerationDisabled(meetingId) {
    return exports.meetingAICostProtector.isMinutesDisabled(meetingId);
}
async function resetMeetingAIUsage(meetingId) {
    return exports.meetingAICostProtector.reset(meetingId);
}
function getAICostConfig() {
    return exports.meetingAICostProtector.getConfig();
}
//# sourceMappingURL=meeting-ai-cost.service.js.map