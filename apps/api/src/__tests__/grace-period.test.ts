...
function makeSubRow(overrides: Record<string, any> = {}) {
  return {
    id: 'sub-1',
...
function setupDbChain(subRow: any | null, planRow: any | null = null) {
...
describe('Grace Period & Subscription Status Logic', () => {
  ...
  describe('getOrgSubscription — status transitions', () => {
    it('should return null when no subscription exists', async () => {
      ...
      const result = await getOrgSubscription('org-1');
  ...
  describe('renewSubscription', () => {
    ...
    it('should extend from current period end if not yet expired', async () => {
      ...
      try {
        await renewSubscription('org-1', 300);
      } catch {
    ...
    it('should extend from now if period already ended', async () => {
      ...
      try {
        await renewSubscription('org-1', 30);
      } catch {
