...
import { QUEUE_NAMES } from '../queues/transcript.queue';
import { getAICostHealthMetrics, AICostAlert } from './ai-cost.monitor';
import { updatePrometheusMetrics, incrementRecoveryMetrics } from './prometheus.metrics';
...
const MONITOR_CONFIG = {
  ...
  queues: [
    QUEUE_NAMES.TRANSCRIPT_EVENTS,
    QUEUE_NAMES.TRANSLATION_JOBS,
...
interface MetricsWindow {
  ...
  minutesGenerationTimes: number[];
  pipelineDelays: number[];
  translationDurations: number[];
}

interface ApiLatencyWindow {
...
class SystemMonitorClass extends EventEmitter {
  ...
  // Worker tracking
  private workerLastCompleted: Map<string, number> = new Map();
  private workerProcessedCounts: Map<string, number> = new Map();
  private workerFailedCounts: Map<string, number> = new Map();

  // Recovery tracking - alerts for jobs that exceeded max retries
  private stuckJobFailedAlerts: WorkerAlert[] = [];
  ...
  private apiLatencyWindow: ApiLatencyWindow = {};
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info('[SYSTEM_MONITOR] Redis connection established');
      ...
      logger.info('[SYSTEM_MONITOR] Initialized successfully', {
        queues: MONITOR_CONFIG.queues,
      });

    } catch (err) {
      logger.error('[SYSTEM_MONITOR] Initialization failed', err);
      throw err;
    }
  }
  ...
  private handleJobCompleted(queueName: string, jobId: string): void {
    ...
    if (queueName === QUEUE_NAMES.TRANSCRIPT_EVENTS) {
      this.metricsWindow.transcriptEvents.push(timestamp);
      this.workerLastCompleted.set('transcript', timestamp);
      this.workerProcessedCounts.set('transcript', 
        (this.workerProcessedCounts.get('transcript') || 0) + 1);
    } else if (queueName === QUEUE_NAMES.TRANSLATION_JOBS) {
      this.metricsWindow.translationEvents.push(timestamp);
      this.workerLastCompleted.set('translation', timestamp);
      this.workerProcessedCounts.set('translation', 
        (this.workerProcessedCounts.get('translation') || 0) + 1);
    } else if (queueName === QUEUE_NAMES.BROADCAST_EVENTS) {
      this.workerLastCompleted.set('broadcast', timestamp);
      this.workerProcessedCounts.set('broadcast', 
        (this.workerProcessedCounts.get('broadcast') || 0) + 1);
    } else if (queueName === QUEUE_NAMES.MINUTES_GENERATION) {
      this.workerLastCompleted.set('minutes', timestamp);
      this.workerProcessedCounts.set('minutes', 
        (this.workerProcessedCounts.get('minutes') || 0) + 1);
    }
  ...
  private handleJobFailed(queueName: string, jobId: string, reason: string): void {
    const workerMap: Record<string, string> = {
      [QUEUE_NAMES.TRANSCRIPT_EVENTS]: 'transcript',
      [QUEUE_NAMES.TRANSLATION_JOBS]: 'translation',
      [QUEUE_NAMES.BROADCAST_EVENTS]: 'broadcast',
      [QUEUE_NAMES.MINUTES_GENERATION]: 'minutes',
    };

    const workerName = workerMap[queueName];
    if (workerName) {
      this.workerFailedCounts.set(workerName, 
        (this.workerFailedCounts.get(workerName) || 0) + 1);
    }

    logger.warn('[SYSTEM_MONITOR] Job failed', {
      queue: queueName,
      jobId,
  ...
  private pruneMetricsWindow(): void {
    const cutoff = Date.now() - 60000; // 1 minute ago

    for (const key of Object.keys(this.metricsWindow) as (keyof MetricsWindow)[]) {
      ...
      try {
        ...
        if (subscriber) {
          try {
            ...
            logger.debug('[SYSTEM_MONITOR] Redis PubSub subscriber closed');
          } catch (cleanupErr) {
            // Force disconnect if quit fails
            try {
              subscriber.disconnect();
              logger.debug('[SYSTEM_MONITOR] Redis PubSub subscriber force disconnected');
            } catch {
   ...
   */
  async checkPostgresHealth(): Promise<PostgresHealth> {
    try {
      const start = Date.now();
      await db.raw('SELECT 1');
      const latencyMs = Date.now() - start;

      const result: PostgresHealth = {
        connected: true,
        latencyMs,
      };
  ...
  async checkQueueHealth(queueName: string): Promise<QueueHealth> {
    ...
    try {
      ...
      const stuckJobDetails: StuckJobInfo[] = [];
      try {
        ...
        if (stuckJobDetails.length > 0) {
          logger.warn('[SYSTEM_MONITOR] Stuck jobs detected', {
            queue: queueName,
   ...
   */
  async checkWorkerHealth(workerName: string): Promise<WorkerHealth> {
    ...
    try {
      ...
      logger.debug('[SYSTEM_MONITOR] Failed to check heartbeat', { workerName, error: (err as Error).message });
    }
   ...
   */
  async sendWorkerHeartbeat(workerName: string): Promise<void> {
    try {
      if (!this.redis) {
        logger.debug('[SYSTEM_MONITOR] Cannot send heartbeat - Redis not initialized');
        return;
      }
      ...
      logger.debug('[SYSTEM_MONITOR] Heartbeat sent', { workerName, timestamp });
    } catch (err) {
      // Non-blocking - just log the error
      logger.debug('[SYSTEM_MONITOR] Failed to send heartbeat', { 
        workerName, 
  ...
  private async getJobRetryCount(jobId: string): Promise<number> {
    try {
      ...
      logger.debug('[SYSTEM_MONITOR] Failed to get job retry count', { 
        jobId, 
        error: (err as Error).message 
      });
      return 0;
   ...
   */
  private async incrementJobRetryCount(jobId: string): Promise<number> {
    try {
      if (!this.redis) return 0;
      const key = `stuck_job_retries:${jobId}`;
      ...
      logger.debug('[SYSTEM_MONITOR] Failed to increment job retry count', { 
        jobId, 
        error: (err as Error).message 
      });
      return 0;
   ...
   */
  private async clearJobRetryCount(jobId: string): Promise<void> {
    try {
      if (!this.redis) return;
      const key = `stuck_job_retries:${jobId}`;
      await this.redis.del(key);
    } catch (err) {
      logger.debug('[SYSTEM_MONITOR] Failed to clear job retry count', { 
        jobId, 
        error: (err as Error).message 
      });
    }
   ...
   */
  async recoverStuckJobs(queueName: string): Promise<RecoveryResult[]> {
    const results: RecoveryResult[] = [];
    const queue = this.queues.get(queueName);
    
    if (!queue) {
      logger.debug('[SYSTEM_MONITOR] Cannot recover - queue not found', { queueName });
      return results;
    }
    ...
    try {
      ...
      for (const job of activeJobs) {
        if (!job.processedOn || !job.id) continue;
        
        const activeForMs = now - job.processedOn;
        if (activeForMs <= maxDuration) continue;
        ...
        if (currentRetries >= maxRetries) {
          ...
          try {
            await job.moveToFailed(
              new Error(`Stuck job exceeded max auto-recovery retries (${maxRetries})`),
            ...
            incrementRecoveryMetrics(queueName, 'failed');
            ...
            logger.error('[SYSTEM_MONITOR] Stuck job moved to failed', {
              jobId: job.id,
              queueName,
              retryCount: currentRetries,
              activeForMs,
            ...
            logger.error('[SYSTEM_MONITOR] Failed to move stuck job to failed', {
              jobId: job.id,
              queueName,
              error: (moveErr as Error).message,
            });
          }
        } else {
          ...
          try {
            ...
            // Update Prometheus counter
            incrementRecoveryMetrics(queueName, 'recovered');

            logger.warn('[SYSTEM_MONITOR] Stuck job recovered', {
              jobId: job.id,
              queueName,
              retryCount: newRetryCount,
              maxRetries,
            ...
            logger.error('[SYSTEM_MONITOR] Failed to recover stuck job', {
              jobId: job.id,
              queueName,
              error: (moveErr as Error).message,
            });
      ...
      if (results.length > 0) {
        logger.info('[SYSTEM_MONITOR] Stuck job recovery completed', {
          queueName,
      ...
      logger.error('[SYSTEM_MONITOR] Failed to recover stuck jobs', {
        queueName,
        error: (err as Error).message,
      });
    }
  ...
  recoverAllStuckJobs(): void {
    ...
    for (const queueName of MONITOR_CONFIG.queues) {
      this.recoverStuckJobs(queueName).catch(err => {
        logger.error('[SYSTEM_MONITOR] Queue recovery failed', {
          queueName,
          error: (err as Error).message,
        });
      });
   ...
   */
  private updatePrometheusFromReport(report: SystemHealthReport): void {
    try {
      ...
      logger.error('[SYSTEM_MONITOR] Failed to update Prometheus metrics', err);
    }
  }
  ...
  getApiLatencyMetrics(): ApiLatencyMetrics[] {
    const metrics: ApiLatencyMetrics[] = [];
  ...
  async generateHealthReport(): Promise<SystemHealthReport> {
    ...
    // Get AI cost metrics
    const aiCost = getAICostHealthMetrics();
  ...
  printHealthReport(report: SystemHealthReport): void {
    ...
    const statusColor = (ok: boolean) => ok ? colors.green : colors.red;
    const formatOk = (ok: boolean, latency?: number) => {
      return ok 
    ...
    if (apiWithRequests.length > 0) {
      const totalAvg = Math.round(
        apiWithRequests.reduce((sum, a) => sum + a.avgLatencyMs, 0) / apiWithRequests.length
      );
    ...
    const statusColorCode = 
      statusText === 'HEALTHY' ? colors.green :
   ...
   */
  async runMonitorCycle(): Promise<SystemHealthReport> {
    try {
      ...
      logger.info('[SYSTEM_MONITOR] Health check completed', {
      ...
      logger.error('[SYSTEM_MONITOR] Monitor cycle failed', err);
      throw err;
    }
  }
  ...
  async start(): Promise<void> {
    if (this.isRunning) {
      logger.warn('[SYSTEM_MONITOR] Already running');
      return;
    }
    ...
    this.intervalId = setInterval(async () => {
      try {
        ...
        logger.error('[SYSTEM_MONITOR] Scheduled cycle failed', err);
      }
  ...
  async stop(): Promise<void> {
    ...
    for (const [name, queueEvents] of queueEventEntries) {
      try {
        ...
        logger.warn(`[SYSTEM_MONITOR] Failed to close queue events for ${name}`, err);
      }
    }
    ...
    for (const [name, queue] of queueEntries) {
      try {
        ...
        logger.warn(`[SYSTEM_MONITOR] Failed to close queue ${name}`, err);
      }
    }
    this.queues.clear();
 ...
 */
export function apiLatencyMiddleware() {
  return (req: any, res: any, next: any) => {
    ...
    res.on('finish', () => {
      ...
      if (duration > MONITOR_CONFIG.thresholds.api.maxLatencyMs) {
        logger.warn('[API_LATENCY] Slow request detected', {
          endpoint,
          duration,
 ...
 */
export function recordBroadcastLatency(latencyMs: number): void {
 ...
 */
export function recordMinutesGenerationTime(durationMs: number): void {
 ...
 */
export function recordPipelineDelay(delayMs: number): void {
 ...
 */
export function recordTranslationDuration(durationMs: number): void {
 ...
 */
export async function sendWorkerHeartbeat(workerName: string): Promise<void> {
 ...
 */
export function triggerStuckJobRecovery(): void {
 ...
 */
export async function recoverStuckJobs(queueName: string): Promise<RecoveryResult[]> {
 ...
 */
export function getStuckJobFailedAlerts(): WorkerAlert[] {
 ...
 */
export function getSystemMonitor(): SystemMonitorClass {
