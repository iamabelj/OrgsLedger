...
import { recordOpenAIUsage } from '../monitoring/ai-cost.monitor';
...
class MinutesAIService {
  ...
  private currentCompletionTokens = 0;

  constructor() {
    // Determine API URL and key
    const apiKey = config.aiProxy?.apiKey || config.ai?.openaiApiKey || '';
    const baseURL = config.aiProxy?.url 
      ? `${config.aiProxy.url}/v1` 
    ...
    if (this.isConfigured) {
      ...
      logger.warn('[MINUTES_AI] No API key configured — AI minutes generation disabled');
    }
  }
   ...
   */
  async generateMinutes(options: GenerateMinutesOptions): Promise<GenerationResult> {
    ...
    logger.info('[MINUTES_AI] Starting minutes generation', {
      meetingId,
    ...
    logger.info('[MINUTES_AI] Transcript formatted', {
      meetingId,
      wordCount,
    ...
    if (formattedTranscript.length > maxChars) {
      logger.info('[MINUTES_AI] Transcript exceeds token limit, using chunking', {
        meetingId,
        transcriptLength: formattedTranscript.length,
        maxChars,
      });
      
      const result = await this.generateWithChunking(
        meetingId,
        formattedTranscript,
        participants,
        model
      ...
      minutes = await this.generateSinglePass(
        meetingId,
        formattedTranscript,
        participants,
        model
    ...
    if (this.currentPromptTokens > 0 || this.currentCompletionTokens > 0) {
      try {
        recordOpenAIUsage(
          this.currentPromptTokens,
          this.currentCompletionTokens,
          undefined, // Use default model from config
          meetingId
        );
        logger.debug('[MINUTES_AI] Token usage recorded', {
          meetingId,
        ...
        logger.warn('[MINUTES_AI] Failed to record token usage', { error: err });
      }
    }
    
    logger.info('[MINUTES_AI] Minutes generation completed', {
      meetingId,
      duration,
      chunksProcessed,
      wordCount,
   ...
   */
  private formatTranscripts(transcripts: TranscriptEntry[]): string {
    // Sort by timestamp
   ...
   */
  private extractParticipants(transcripts: TranscriptEntry[]): string[] {
...
Generate meeting minutes as JSON.`;
    ...
    try {
      ...
      logger.warn('[MINUTES_AI] Single-pass generation failed, using fallback', {
        meetingId,
        error: err.message,
      });
      return this.generateFallbackMinutes(transcript, participants);
  ...
  private async generateWithChunking(
    ...
    participants: string[],
    model?: string
  ): Promise<{ minutes: StructuredMinutes; chunksProcessed: number }> {
    // Split transcript into chunks
    const chunks = this.splitIntoChunks(
      transcript,
      MINUTES_CONFIG.chunkSize,
      MINUTES_CONFIG.overlapSize
    );

    logger.info('[MINUTES_AI] Chunking transcript', {
      meetingId,
    ...
    for (let i = 0; i < chunks.length; i++) {
      logger.debug('[MINUTES_AI] Processing chunk', {
        meetingId,
        chunk: i + 1,
        totalChunks: chunks.length,
      });
      ...
      try {
        ...
        logger.warn('[MINUTES_AI] Chunk processing failed', {
          meetingId,
          chunk: i + 1,
          error: err.message,
        });
    ...
    if (chunkSummaries.length === 0) {
      logger.warn('[MINUTES_AI] All chunks failed, using fallback', { meetingId });
      return {
        minutes: this.generateFallbackMinutes(transcript, participants),
        chunksProcessed: chunks.length,
      };
    ...
    const finalMinutes = await this.combineChunkSummaries(
      meetingId,
      chunkSummaries,
      participants,
      model
   ...
   */
  private async combineChunkSummaries(
    meetingId: string,
    summaries: any[],
    participants: string[],
    model?: string
    ...
    try {
      ...
      summaries.forEach(s => {
        if (Array.isArray(s.participants)) {
          s.participants.forEach((p: string) => allParticipants.add(p));
        }
      });
      ...
      logger.warn('[MINUTES_AI] Final summary failed, merging chunks manually', {
        meetingId,
        error: err.message,
      });
  ...
  private mergeChunkSummaries(
    ...
    summaries.forEach(s => {
      if (Array.isArray(s.keyPoints)) allKeyPoints.push(...s.keyPoints);
      if (Array.isArray(s.decisions)) allDecisions.push(...s.decisions);
  ...
  private async callLLM(prompt: string, model?: string): Promise<string> {
    ...
    try {
      ...
      if (err instanceof OpenAI.APIError) {
        logger.warn('[MINUTES_AI] OpenAI API error', {
          status: err.status,
          message: err.message,
          code: err.code,
        });
   ...
   */
  private normalizeActionItems(items: any): ActionItem[] {
...
let minutesAIService: MinutesAIService | null = null;

/**
 * Get the AI minutes service instance
 */
export function getMinutesAIService(): MinutesAIService {
  if (!minutesAIService) {
 ...
 */
export function isMinutesAIAvailable(): boolean {
