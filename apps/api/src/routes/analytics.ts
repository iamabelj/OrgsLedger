...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const meetingsWithAI = await db('meetings')
        .where({ organization_id: orgId, ai_enabled: true })
        .where('status', 'ended')
        .count('id as count')
        .first();

      const minutesGenerated = await db('meeting_minutes')
      ...
      const totalAiCreditsUsed = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
      ...
      const meetingFrequency = await db('meetings')
        .where({ organization_id: orgId })
        .where('scheduled_start', '>=', lastSixMonths)
        .select(db.raw("DATE_TRUNC('month', scheduled_start) as month"))
        .count('id as count')
        .groupBy('month')
        .orderBy('month', 'asc');
      ...
      // ── Average Attendance ──
      const attendanceStats = await db('meeting_attendance')
        .join('meetings', 'meeting_attendance.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meetings.status': 'ended' })
        .select(db.raw(`
      ...
      const decisionsCount = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
        .select(db.raw(`
      ...
      const actionItemsCount = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
        .select(db.raw(`
      ...
      const actionItemsByPriority = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
        .select(db.raw(`
      ...
      actionItemsByPriority.forEach((row: any) => {
        const priority = row.priority?.toLowerCase() || 'medium';
        if (priority in priorityCounts) {
          priorityCounts[priority as keyof typeof priorityCounts] = parseInt(row.count) || 0;
        }
      });

      const motionsCount = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
        .select(db.raw(`
      ...
      const topContributors = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
        .select(db.raw(`
      ...
      const recentMetrics = await db('meetings')
        .where({ organization_id: orgId, status: 'ended' })
        .where('actual_end', '>=', lastThirtyDays)
        .count('id as meetings_last_30_days')
        .first();

      const recentMinutes = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
      ...
      const minutesTrend = await db('meeting_minutes')
        .join('meetings', 'meeting_minutes.meeting_id', 'meetings.id')
        .where({ 'meetings.organization_id': orgId, 'meeting_minutes.status': 'completed' })
        .where('meeting_minutes.generated_at', '>=', lastSixMonths)
        .select(db.raw("DATE_TRUNC('month', meeting_minutes.generated_at) as month"))
        .count('meeting_minutes.id as count')
      ...
      res.json({
        ...
        data: {
          overview: {
            ...
            meetingsWithAI: parseInt(meetingsWithAI?.count as string) || 0,
            minutesGenerated: parseInt(minutesGenerated?.count as string) || 0,
          ...
          recent: {
            meetingsLast30Days: parseInt(recentMetrics?.meetings_last_30_days as string) || 0,
            minutesLast30Days: parseInt(recentMinutes?.minutes_last_30_days as string) || 0,
          },
        },
      });
    } catch (err) {
