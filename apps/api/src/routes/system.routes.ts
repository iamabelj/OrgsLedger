...
import { getHealthReport } from '../monitoring/system.monitor';
import { getMeetingMetrics } from '../monitoring/meeting-metrics';
import { getMetricsString, getRegistry } from '../monitoring/prometheus.metrics';
import { getWorkerHeartbeatStats } from '../monitoring/worker-heartbeat.monitor';
import { getAllBackpressureStatus } from '../scaling/backpressure';
import { getAIRateLimitMetrics } from '../monitoring/ai-rate-limit.guard';
import { getRedisShardStats } from '../infrastructure/redisShardRouter';
import { getQueueMetricsExporter } from '../monitoring/queue-metrics.exporter';
import { cleanupMeeting, autoCleanupStaleMeetings } from '../services/meeting-cleanup.service';
import { getCostGuardStatus } from '../middleware/cost-guard.middleware';
import { 
  SHARDED_QUEUE_TYPES, 
  type ShardedQueueType,
  queueManager,
  getDLQStats,
  getShardStats,
} from '../queues/queue-manager';
import { logger } from '../logger';
import { WORKER_ID, CPU_CORES } from '../scaling/worker-identity';
import { getBroadcastBatchStats } from '../scaling/broadcast-batch';
import { getWSThrottleStats } from '../scaling/ws-throttle';
...
interface HealthResponse {
  ...
  queues: {
    transcript: QueueStatus;
    translation: QueueStatus;
    broadcast: QueueStatus;
    minutes: QueueStatus;
  };
  workers: {
    transcript: WorkerStatus;
    translation: WorkerStatus;
    broadcast: WorkerStatus;
    minutes: WorkerStatus;
  };
  pipeline: {
...
type WorkerStatus = 'running' | 'inactive' | 'unhealthy';

// ── Constants ─────────────────────────────────────────────

// Queue name mapping from internal names to response keys
const QUEUE_NAME_MAP: Record<string, keyof HealthResponse['queues']> = {
...
router.get('/health', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    const reportPromise = getHealthReport();
    ...
    if (!report) {
      logger.warn('[SYSTEM_HEALTH] Health check timed out');
    ...
    const queues: HealthResponse['queues'] = {
      transcript: { waiting: 0, active: 0, failed: 0, paused: false, stuckJobs: 0 },
      translation: { waiting: 0, active: 0, failed: 0, paused: false, stuckJobs: 0 },
      broadcast: { waiting: 0, active: 0, failed: 0, paused: false, stuckJobs: 0 },
      minutes: { waiting: 0, active: 0, failed: 0, paused: false, stuckJobs: 0 },
    ...
    for (const w of report.workers) {
      const key = w.name as keyof HealthResponse['workers'];
      if (key in workers) {
        if (!w.healthy) {
    ...
    logger.error('[SYSTEM_HEALTH] Health check failed', { error: err.message });
 ...
 */
router.get('/meeting-metrics/:meetingId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get meeting metrics', {
      meetingId,
      error: err.message,
    });
...
router.get('/workers', async (_req: Request, res: Response) => {
  try {
    const stats = await getWorkerHeartbeatStats();
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get worker stats', {
      error: err.message,
    });
 ...
 */
router.get('/workers/:workerName', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get worker stats by name', {
      error: err.message,
    });
 ...
 */
router.get('/backpressure', async (_req: Request, res: Response) => {
  try {
    const status = await getAllBackpressureStatus();
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get backpressure status', {
      error: err.message,
    });
 ...
 */
router.get('/ai-rate-limits', async (_req: Request, res: Response) => {
  try {
    const metrics = await getAIRateLimitMetrics();
    ...
    res.status(200).json({
      ...
      summary: {
        ...
        highestUtilization: Math.max(
          metrics.deepgram.utilizationPercent,
          metrics.openai.utilizationPercent,
          metrics.translate.utilizationPercent
        ),
        servicesInWarning: [
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get AI rate limit status', {
      error: err.message,
    });
 ...
 * - Remaining budget
 */
router.get('/cost-guard', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get cost guard status', {
      error: err.message,
    });
 ...
 */
router.get('/redis-shards', async (_req: Request, res: Response) => {
  try {
    const stats = await getRedisShardStats();
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get Redis shard stats', {
      error: err.message,
    });
...
router.get('/queue-metrics', async (_req: Request, res: Response) => {
  try {
    const exporter = getQueueMetricsExporter();
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get queue metrics', {
      error: err.message,
    });
 ...
 */
router.get('/queue-metrics/:queueType', async (req: Request, res: Response) => {
  try {
    ...
    const exporter = getQueueMetricsExporter();
    const stats = exporter.getQueueStats(queueType as ShardedQueueType);
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get queue type metrics', {
      error: err.message,
...
router.get('/metrics', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error('[SYSTEM_ROUTES] Failed to generate Prometheus metrics', {
      error: err.message,
    });
 ...
 * - Capacity planning
 */
router.get('/internal/metrics', async (_req: Request, res: Response) => {
  const startTime = Date.now();
  ...
  try {
    ...
    const [
      ...
      getShardStats(SHARDED_QUEUE_TYPES.TRANSCRIPT_EVENTS),
      getShardStats(SHARDED_QUEUE_TYPES.TRANSLATION_JOBS),
      getShardStats(SHARDED_QUEUE_TYPES.BROADCAST_EVENTS),
      getShardStats(SHARDED_QUEUE_TYPES.MINUTES_GENERATION),
      getDLQStats(SHARDED_QUEUE_TYPES.TRANSCRIPT_EVENTS),
      getDLQStats(SHARDED_QUEUE_TYPES.TRANSLATION_JOBS),
      getDLQStats(SHARDED_QUEUE_TYPES.BROADCAST_EVENTS),
      getDLQStats(SHARDED_QUEUE_TYPES.MINUTES_GENERATION),
      getHealthReport().catch(() => null),
      getWorkerHeartbeatStats().catch(() => null),
      getAllBackpressureStatus().catch(() => ({})),
      getAIRateLimitMetrics().catch(() => null),
    ]);
    ...
    try {
      broadcastBatchStats = getBroadcastBatchStats();
      wsThrottleStats = getWSThrottleStats();
    } catch {
    ...
    logger.error('[SYSTEM_ROUTES] Failed to get internal metrics', {
      error: err.message,
    });
...
function sumTotalsField(stats: any[], field: string): number {
 ...
 */
router.post('/meeting-cleanup/:meetingId', async (req: Request, res: Response) => {
  try {
    const { meetingId } = req.params;
    const { organizationId } = req.body;

    const result = await cleanupMeeting(meetingId, organizationId);
    ...
    logger.error('[SYSTEM_ROUTES] Meeting cleanup failed', {
      error: err.message,
 ...
 */
router.post('/meeting-cleanup/stale', async (req: Request, res: Response) => {
  try {
    const maxAgeHours = parseInt(req.body.maxAgeHours || '24', 10);

    const result = await autoCleanupStaleMeetings(maxAgeHours);
    ...
    logger.error('[SYSTEM_ROUTES] Stale meeting cleanup failed', {
      error: err.message,
    });
 ...
 */
router.get('/dependency-check', async (_req: Request, res: Response) => {
  try {
    ...
    try {
      const { redisClientManager } = await import('../infrastructure/redisClient');
    ...
    for (const pkg of packageChecks) {
      try {
        ...
        const mod = await import(pkg.importPath);
    ...
    logger.error('[SYSTEM_ROUTES] Dependency check failed', {
      error: err.message,
    });
