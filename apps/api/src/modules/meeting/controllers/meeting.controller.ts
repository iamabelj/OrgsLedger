...
} from '../services/livekit-token.service';
...
function formatMeetingResponse(meeting: any) {
  return {
    ...
    // Computed fields
    participantCount: meeting.participants.filter((p: any) => !p.leftAt).length,
...
export class MeetingController {
  ...
  async join(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const meeting = await meetingService.join(meetingId, userId, displayName);
      ...
      logger.error('[MEETING_CONTROLLER] Join failed', { 
        error: error.message,
        userId: req.user?.userId,
      });
  ...
  async leave(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const meeting = await meetingService.leave(meetingId, userId);
      ...
      logger.error('[MEETING_CONTROLLER] Leave failed', { 
        error: error.message,
        userId: req.user?.userId,
      });
  ...
  async getById(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const meeting = await meetingService.getByIdWithState(id);
      ...
      logger.error('[MEETING_CONTROLLER] GetById failed', { 
        error: error.message,
  ...
  async list(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await meetingService.listByOrganization(
      ...
      logger.error('[MEETING_CONTROLLER] List failed', { error: error.message });
      next(error);
    }
  }
  ...
  async start(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const meeting = await meetingService.start(id, userId);
      ...
      logger.error('[MEETING_CONTROLLER] Start failed', { 
        error: error.message,
        meetingId: req.params.id,
      });
  ...
  async end(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const meeting = await meetingService.end(id, userId);
      ...
      logger.error('[MEETING_CONTROLLER] End failed', { 
        error: error.message,
        meetingId: req.params.id,
      });
  ...
  async cancel(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const meeting = await meetingService.cancel(id, userId);
      ...
      logger.error('[MEETING_CONTROLLER] Cancel failed', { 
        error: error.message,
        meetingId: req.params.id,
      });
  ...
  async getToken(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      await createRoomIfNotExists(id);
      ...
      logger.info('[MEETING_CONTROLLER] Token generated', {
        meetingId: id,
        userId,
        role,
      });
      ...
      logger.error('[MEETING_CONTROLLER] GetToken failed', {
        error: error.message,
        meetingId: req.params.id,
        userId: req.user?.userId,
      });
  ...
  async getMinutes(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const minutes = await meetingService.getMinutes(id);
      ...
      logger.info('[MEETING_CONTROLLER] Minutes retrieved', {
        meetingId: id,
        userId,
      ...
      logger.error('[MEETING_CONTROLLER] GetMinutes failed', {
        error: error.message,
        meetingId: req.params.id,
        userId: req.user?.userId,
      });
   ...
   */
  async regenerateMinutes(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user!.userId;
      const { id } = req.params;
      ...
      await meetingService.resubmitMinutesJob(id, meeting.organizationId);

      logger.info('[MEETING_CONTROLLER] Minutes regeneration requested', {
        meetingId: id,
        userId,
      });
      ...
      logger.error('[MEETING_CONTROLLER] RegenerateMinutes failed', {
        error: error.message,
        meetingId: req.params.id,
        userId: req.user?.userId,
      });
