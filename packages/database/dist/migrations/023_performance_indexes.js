...
async function up(knex) {
    ...
    if (!(await knex.schema.hasTable('meeting_attendance')))
        return;
    ...
    if (await knex.schema.hasTable('announcements')) {
        await knex.schema.alterTable('announcements', (t) => {
            t.index(['organization_id', 'pinned', 'created_at'], 'idx_announcements_org_pinned_created');
        });
    }
    // ── LOW priority ───────────────────────────────────────
...
//# sourceMappingURL=023_performance_indexes.js.map