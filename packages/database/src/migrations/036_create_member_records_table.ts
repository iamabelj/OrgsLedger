...
export async function up(knex: Knex): Promise<void> {
  ...
  console.log('[Migration 036] Created member_records table');
}

export async function down(knex: Knex): Promise<void> {
  // Drop trigger first
  await knex.raw(`
    DROP TRIGGER IF EXISTS trigger_member_records_updated_at ON member_records;
    DROP FUNCTION IF EXISTS update_member_records_updated_at();
  `);

  await knex.schema.dropTableIfExists('member_records');
  console.log('[Migration 036] Dropped member_records table');
}
