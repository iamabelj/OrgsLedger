...
type PlanSeed = {
  ...
  price_ngn_monthly: number | null;
  sort_order: number;
  description: string;
  includedAiHours?: number;
...
function withIncludedAiHours(features: Record<string, any>, hours?: number): Record<string, any> {
  const next = { ...features };
...
async function upsertPlan(knex: Knex, seed: PlanSeed) {
  const existing = await knex('subscription_plans').where({ slug: seed.slug }).first();
  const mergedFeatures = withIncludedAiHours(
    { ...(parseFeatures(existing?.features)), ...(seed.baseFeatures || {}) },
    seed.includedAiHours
  );
...
async function removeIncludedHoursFromPlan(knex: Knex, slug: string) {
